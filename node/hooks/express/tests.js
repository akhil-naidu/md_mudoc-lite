  ...
  app.get('/tests/frontend/frontendTestSpecs.json', (req, res, next) => {
    (async () => {
      ...
      await Promise.all(Object.entries(plugins.plugins).map(async ([plugin, def]) => {
        ...
        const specDir = `${plugin === 'md_mudoc-lite' ? '' : 'static/'}tests/frontend/specs`;
        for (const spec of await findSpecs(path.join(pluginPath, specDir))) {
          if (plugin === 'md_mudoc-lite' && !settings.enableAdminUITests &&
      ...
      modules.sort((a, b) => {
        a = String(a);
        b = String(b);
        const aCore = a.startsWith('md_mudoc-lite/');
        const bCore = b.startsWith('md_mudoc-lite/');
