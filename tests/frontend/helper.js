...
(() => {
  ...
  helper.clearPadPrefCookie = () => {
    const {padcookie} = helper.padChrome$.window.require('md_mudoc-lite/static/js/pad_cookie');
    padcookie.clear();
  };
  ...
  helper.aNewPad = async (opts = {}) => {
    ...
    await Promise.all([
      ...
      new Promise((resolve, reject) => {
        ...
        $iframe[0].contentWindow._postPluginUpdateForTesting = () => {
          const {hooks} =
                $iframe[0].contentWindow.require('md_mudoc-lite/static/js/pluginfw/plugin_defs');
    ...
    helper.padChrome$.padeditor =
        helper.padChrome$.window.require('md_mudoc-lite/static/js/pad_editor').padeditor;
    if (opts.clearCookies) {
      helper.clearPadPrefCookie();
    }
    if (opts.padPrefs) {
