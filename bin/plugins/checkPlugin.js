...
(async () => {
  ...
  if (!files.includes('package.json')) {
    ...
    await updateDeps(parsedPackageJSON, 'peerDependencies', {
      ...
      'md_mudoc-lite': {ver: '>=1.8.6', overwrite: false},
    });
  ...
  if (readMeFileName) {
    ...
    const publishBadge = `![Publish Status](https://github.com/akhil-naidu/${pluginName}/workflows/Node.js%20Package/badge.svg)`;
    // eslint-disable-next-line max-len
    const testBadge = `![Backend Tests Status](https://github.com/akhil-naidu/${pluginName}/workflows/Backend%20tests/badge.svg)`;
  ...
  if (files.includes('static')) {
    ...
    if (!staticFiles.includes('tests')) {
      console.warn('Test files not found, please create tests.  https://github.com/akhil-naidu/mudoc-lite/wiki/Creating-a-plugin#writing-and-running-front-end-tests-for-your-plugin');
    }
  } else {
    console.warn('Test files not found, please create tests.  https://github.com/akhil-naidu/mudoc-lite/wiki/Creating-a-plugin#writing-and-running-front-end-tests-for-your-plugin');
  }
  ...
  // The md_mudoc-lite peer dep must be installed last otherwise `npm install` will nuke it. An
  // absolute path to mudoc-lite/src is used here so that pluginPath can be a symlink.
  execSync(
      `${npmInstall} --no-save md_mudoc-lite@file:${__dirname}/../../`, {stdio: 'inherit'});
  // linting begins
  try {
    console.log('Linting...');
