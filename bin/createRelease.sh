...
ETHER_REPO="https://github.com/akhil-naidu/mudoc-lite.git"
ETHER_WEB_REPO="https://github.com/akhil-naidu/akhil-naidu.github.com.git"
TMP_DIR="/tmp/"
...
function merge_release_branch {
  ...
  CURL_RESPONSE=$(curl --silent -i -N --data "$API_JSON" https://api.github.com/repos/akhil-naidu/mudoc-lite/merges?access_token=$API_TOKEN  | iconv -f utf8)
  echo $CURL_RESPONSE
  HTTP_STATUS=$(echo $CURL_RESPONSE | head -1 | sed -r 's/.* ([0-9]{3}) .*/\1/')
...
function create_builds {
  echo "Cloning mudoc-lite repo and akhil-naidu.github.com repo..."
  cd $TMP_DIR
  rm -rf mudoc-lite akhil-naidu.github.com
...
function push_builds {
  cd $TMP_DIR/mudoc-lite/
  echo "Copying windows build and docs to website repo..."
  GIT_SHA=$(git rev-parse HEAD | cut -c1-10)
  mv mudoc-lite-win.zip $TMP_DIR/akhil-naidu.github.com/downloads/mudoc-lite-win-$VERSION-$GIT_SHA.zip

  mv out/doc $TMP_DIR/akhil-naidu.github.com/doc/v$VERSION

  cd $TMP_DIR/akhil-naidu.github.com/
  sed -i "s/mudoc-lite-win.*\.zip/mudoc-lite-win-$VERSION-$GIT_SHA.zip/" index.html
...
function merge_web_branch {
        ...
        API_JSON=$(printf '{"base": "master","head": "release_%s","commit_message": "Release version %s"}' $VERSION $VERSION)
         CURL_RESPONSE=$(curl --silent -i -N --data "$API_JSON" https://api.github.com/repos/akhil-naidu/akhil-naidu.github.com/merges?access_token=$API_TOKEN | iconv -f utf8)
  echo $CURL_RESPONSE
  HTTP_STATUS=$(echo $CURL_RESPONSE | head -1 | sed -r 's/.* ([0-9]{3}) .*/\1/')
...
function publish_release {
  ...
  if [ $PUBLISH_RELEASE = "y" ]; then
    ...
    CURL_RESPONSE=$(curl --silent -i -N --data "$API_JSON" https://api.github.com/repos/akhil-naidu/mudoc-lite/releases?access_token=$API_TOKEN | iconv -f utf8)
    HTTP_STATUS=$(echo $CURL_RESPONSE | head -1 | sed -r 's/.* ([0-9]{3}) .*/\1/')
...
function todo_notification {
  ...
  echo "- Update the wiki at https://github.com/akhil-naidu/mudoc-lite/wiki"
