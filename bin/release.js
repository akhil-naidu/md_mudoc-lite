...
if (!dirExists('../akhil-naidu.github.com')) {
  throw new Error('please clone documentation repo: ' +
                  '(cd .. && git clone git@github.com:akhil-naidu/akhil-naidu.github.com.git)');
}
assertWorkDirClean({cwd: '../akhil-naidu.github.com/'});
assertBranchCheckedOut('master', {cwd: '../akhil-naidu.github.com/'});
assertUpstreamOk('master', {cwd: '../akhil-naidu.github.com/'});
...
try {
  ...
  run('make docs');
  console.log('Updating akhil-naidu.github.com master branch...');
  run('git pull --ff-only', {cwd: '../akhil-naidu.github.com/'});
  console.log('Committing documentation...');
  run(`cp -R out/doc/ ../akhil-naidu.github.com/doc/v'${newVersion}'`);
  run(`rm -f latest && ln -s 'v${newVersion}' latest`, {cwd: '../akhil-naidu.github.com/doc/'});
  run('git add .', {cwd: '../akhil-naidu.github.com/'});
  run(`git commit -m '${newVersion} docs'`, {cwd: '../akhil-naidu.github.com/'});
} catch (err) {
  console.error(err.toString());
  console.warn('Resetting repository...');
  console.warn('Resetting master...');
  run('git checkout -f master', {cwd: '../akhil-naidu.github.com/'});
  run('git reset --hard @{u}', {cwd: '../akhil-naidu.github.com/'});
  throw err;
}
...
console.log('  (cd ../akhil-naidu.github.com && git show)');
console.log('If everything looks good then push:');
console.log(`  git push origin master develop '${newVersion}'`);
console.log('  (cd ../akhil-naidu.github.com && git push)');
console.log('Create a Windows build:');
console.log('  bin/buildForWindows.sh');
console.log('Visit https://github.com/akhil-naidu/mudoc-lite/releases/new and create a new release ' +
